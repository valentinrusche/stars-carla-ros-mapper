...
from carla_actor_state_types.msg import CarlaActorState
from ..async_service_client import AsyncServiceClient
from ...util.math_operations import rpy_from_quaternion


class CarlaDynamicInfoClient(AsyncServiceClient):

    def __init__(self, node_name: str, message_type, topic_name: str, callback_group,
                 tick_time = 0.05, role_name = "ego_vehicle", file_dir = os.getenv(key = "CARLA_STARS_DYNAMIC_FILE_DIR")) -> None:
        """Requests all dynamic data available from the carla-ros-bridge by polling the data every tick"""
        ...
        self.results: List[CarlaActorState] = []
    ...
    def __handle_actors(self, actors) -> None:
        for actor in actors:
            ...
            if not self.received_actor_callback:
                self.get_logger().info(message = f"Found Actor(id={actor.id}) via subscription on topic '/carla/all_vehicle_actors'.")
    ...
    def send_request(self, id: int):
        # equivalent to: ros2 service call /carla_actor_state_publisher/get_actor_state carla_actor_state_types/srv/GetActorState '{id: <id>}'
        ...
        for result in self.results:
            if self.save_counter % 100 == 0:
                if result is not None:
                    self.get_logger().info(message=f"Received 100 CarlaActorStates. Current tick is {str(object = result.current_tick)}.")
